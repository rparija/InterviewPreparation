...
public class CoinChangeMinCoinDP {
    ...
    public static void main(String[] args) {
        int n=11;
        int []s={1,2,5};
        System.out.println("Minimum number of coins to make the change: " + coinChangeTopDownApproach(s,n));
    }
...
public static int coinChangeTopDownApproach(int[] s, int n) {
    ...
    /*initializing the 1st row to INT max and 1st column to 0 in a matrix*/
    ...
    /*For this problem we need to initialize the second row as well
    * if the second row elements is divided by first element in array
    * then populate that value with the quotient otherwise populate with int max val*/
    for (int j=1;j<n+1;j++)
    {
        if(j%s[0]==0)
        {
